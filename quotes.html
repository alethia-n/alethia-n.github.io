...
<title>Citations + Chat Privé Sécurisé</title>
...
<input type="password" id="signupPassword" placeholder="Mot de passe (≥6 caractères)" required>
...
import { collection, addDoc, query, orderBy, onSnapshot, getDocs, doc, setDoc, getDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/11.0.1/firebase-firestore.js";
...
// 🔹 Charger l'UID admin depuis Firestore
async function loadAdminUid() {
  try {
    const adminDoc = await getDoc(doc(db, "config", "admin"));
    if (adminDoc.exists()) {
      ADMIN_UID = adminDoc.data().uid;
      console.log("✅ UID admin chargé :", ADMIN_UID);
    } else {
      console.error("❌ Aucun document 'admin' trouvé !");
    }
  } catch (err) {
    console.error("Erreur chargement UID admin :", err);
  }
}
await loadAdminUid();
...
sendBtn.addEventListener("click", async () => {
  ...
  if (currentUserUid === ADMIN_UID && !recipientUid) {
  ...
  let senderName =
    currentUserUid === ADMIN_UID
      ? "Admin"
      : usersDivMap[currentUserUid]?.firstName + " " + usersDivMap[currentUserUid]?.lastName;
  try {
    ...
    if (currentUserUid !== ADMIN_UID) alert("Message envoyé !");
  } catch (err) {
